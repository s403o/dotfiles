...
function write_entire_file(path, content) {
...
function open_web_uri_in_default_app(uri) {
    uri = uri.trim();
    if (uri.indexOf(':') === -1)
        uri = 'https://' + uri;
...
function open_file_in_default_app(path) {
...
function open_file_dialog(select_dirs, start, callback) {
    try {
...
var FileMonitor = class FileMonitor {
    is_closed = true;
