...
const { get_iso_date } = _ME.imports.utils.time;
const { _, unreachable } = _ME.imports.utils.misc;
const { Markup } = _ME.imports.utils.markup.renderer;
const { EditorView } = _ME.imports.utils.markup.editor;
const { ButtonBox, Button, CheckBox } = _ME.imports.utils.button;
...
var TaskCard = class TaskCard extends Misc.Card {
    constructor(applet, task, body) {
        ...
        if (config.created)
            this.left_header_box.add_actor(new St.Label({ text: _('Created') + ' ' + config.created, y_align: Clutter.ActorAlign.CENTER, style: 'font-weight: bold', style_class: 'cronomix-green' }));
...
var TaskEditor = class TaskEditor extends EditorView {
    #task;
    #applet;
    #tags = null;
